...
import com.notifa.ai.util.AppUsageData
import com.notifa.ai.util.UsageStatsHelper
    ...
    fun loadInsights() {
        viewModelScope.launch {
            repository.getAllNotifications().collect { notifications ->
                ...
                val batchedCount = todayNotifications.count { it.category == NotificationCategory.BATCHED }
                val instantCount = todayNotifications.count { it.category == NotificationCategory.INSTANT }
                ...
                val topApps = if (UsageStatsHelper.hasUsageStatsPermission(context)) {
                    ...
                    notificationCounts.entries
                        ...
                        .take(5)
                        .map { entry ->
                            val appName = entry.key.split(".").lastOrNull()?.capitalize() ?: entry.key

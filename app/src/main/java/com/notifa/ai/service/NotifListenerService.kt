...
class NotifListenerService : NotificationListenerService() {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification?) {
        ...
        sbn?.notification?.let { notification ->
            ...
            val priority = when {
                packageName.contains("dialer") || packageName.contains("phone") -> Priority.HIGH
                packageName.contains("messaging") || packageName.contains("sms") -> Priority.HIGH
                packageName.contains("whatsapp") -> Priority.HIGH
                else -> Priority.NORMAL
            }

            // Perform DB access inside coroutine (suspend functions)
            scope.launch {
                ...
                if (total > 0) {
                    ...
                    val next = "2h"
                    val summaryLine = if (batchedCount >= 3) "$batchedCount batched notifications" else null
                    NotificationHelper.updateSummaryNotification(applicationContext, total, batchedCount, instantCount, next, summaryLine)
                }
            }
        }
    }
